...
C 是面向过程、编译型语言，无内置容器与哈希表，LeetCode 中常用数组、指针、结构体与 malloc/free 实现数据结构。下面简要回顾与刷题相关的内容：数组、指针、字符串与内存管理。

## LeetCode 使用建议

- 优先写清晰的函数接口：入参与返回值按题目签名对齐；
- 涉及链表/树优先关注内存生命周期，避免悬空指针；
- 需要哈希时可使用 UTHash；
- 处理数组题优先“下标 + 双指针”，减少不必要内存分配。
