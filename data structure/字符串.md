...
字符串可以作为一个整体被访问和使用，也可以用下标，如 s[i] 的形式**访问**其中某一特定位置的字符。
...
KMP 用于在文本串 S 中查找模式串 P 的首次出现位置，时间复杂度 O(|S|+|P|)。核心思想：当某位失配时，利用**最长公共前后缀**将模式串右移，文本串指针不回退。

- **next[j]**：表示模式串 P[0..j-1] 的真前缀与真后缀相等时的最大长度（长度小于 j）。失配时令 j = next[j]，相当于把 P 的已匹配前缀中与后缀重合的部分移到当前对齐位置。
- 直接移动模式串，使「已比较部分」的前缀与后缀对齐，从而避免暴力回退。

```C++
// 构造 next 表：next[i] 表示 P[0..i-1] 的最长公共真前后缀长度
...
// KMP 匹配：返回 S 中首次出现 P 的起始下标，未找到返回 -1
int match(char* P, char* S) {
    int* next = buildNext(P);
    int m = (int) strlen(S), i = 0;
    int n = (int) strlen(P), j = 0;
    while (i < m && j < n) {
        if (j < 0 || S[i] == P[j]) {
            i++; j++;
        } else {
